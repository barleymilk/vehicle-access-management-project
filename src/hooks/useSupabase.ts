...
// People 테이블 데이터 수정 함수
export async function updatePersonToSupabase(
  personId: string,
  personData: {
    name?: string;
    organization?: string;
  ...
  try {
    ...
    // 날짜 필드 처리
    if (personData.activity_start_date) {
      dataToUpdate.activity_start_date =
        personData.activity_start_date.toISOString();
    }
    if (personData.activity_end_date) {
      dataToUpdate.activity_end_date =
        personData.activity_end_date.toISOString();
    }
    ...
    console.error("People 테이블 데이터 수정 중 오류:", error);
    return { data: null, error };
  }
}
